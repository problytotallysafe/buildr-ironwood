...
import { AlertTriangle, CalendarDays, Check, CircleDollarSign, ClipboardCheck, Clock3, ExternalLink, PhoneCall, Plus, RotateCcw, Trash2 } from "lucide-react";
...
export async function ProjectTodayContent({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  const [{ data: projects }, { data: completedProjects }, { data: tasks }, { data: selections }, { data: milestones }, { data: punchItems }, { data: callbacks }, { data: activeTime }, { data: milestonePayments }] = await Promise.all([
    ...
    supabase.from("project_callbacks").select("id,callback_number,title,status,warranty_status,reported_at,scheduled_for,projects(id,name)").in("status", ["draft", "accepted"]).is("archived_at", null).is("deleted_at", null).order("scheduled_for", { ascending: true, nullsFirst: false }).order("reported_at", { ascending: false }),
  ...
  const [{ data: completedChangeOrders }, { data: completedCallbackCharges }, { data: completedPayments }, { data: paymentActivity }] = await Promise.all([
    completedIds.length
      ? supabase.from("change_orders").select("project_id,total,status").in("project_id", completedIds).eq("status", "accepted")
      : Promise.resolve({ data: [] } as any),
    completedIds.length
      ? supabase.from("project_callbacks").select("project_id,homeowner_amount").in("project_id", completedIds).in("status", ["accepted", "completed"]).is("deleted_at", null).gt("homeowner_amount", 0)
      : Promise.resolve({ data: [] } as any),
    completedIds.length
  ...
  const overdueInvoices = (completedProjects ?? []).map((project: any) => {
    ...
    const callbackCharges = (completedCallbackCharges ?? []).filter((item: any) => item.project_id === project.id).reduce((sum: number, item: any) => sum + Number(item.homeowner_amount), 0);
    ...
    return { ...project, estimate, balance: Math.max(0, base + changes + callbackCharges - paid), sentAt: latestSentAt(project.id) };
  ...
  return <section className={embedded ? "dashboard-today" : undefined}>
    ...
    <div className="today-grid"><div className="stack">
      ...
      {((callbacks ?? []).length > 0 || (selections ?? []).length > 0 || dueMilestones.length > 0 || (punchItems ?? []).length > 0) && <section className="panel"><div className="panel-heading"><div><h2>Automatic second set of eyes</h2>{!embedded && <p>Buildr found callbacks and dated commitments that need attention.</p>}</div><AlertTriangle/></div><div className="today-watch-list">{(callbacks ?? []).map((item: any) => <article key={`callback-${item.id}`}><PhoneCall/><div><strong>{item.title} · {item.projects?.name || "Completed project"}</strong><span>{item.callback_number} · {item.status === "draft" ? "Needs review" : "Repair accepted"} · {String(item.warranty_status).replaceAll("_", " ")}{item.scheduled_for ? ` · Scheduled ${new Date(`${item.scheduled_for}T12:00:00`).toLocaleDateString()}` : ""}</span></div><Link href={`/callbacks/${item.id}`}>Open</Link></article>)}{(punchItems ?? []).map((item: any) => <article key={item.id}><ClipboardCheck/><div><strong>{item.description}</strong><span>Punch list{item.room_location ? ` · ${item.room_location}` : ""} · {item.responsible_party}{item.due_date ? ` · Due ${new Date(`${item.due_date}T12:00:00`).toLocaleDateString()}` : ""}</span></div><Link href={`/projects/${item.project_id}#closeout`}>Open</Link></article>)}{(selections ?? []).map((item: any) => <article key={item.id}><CalendarDays/><div><strong>{item.description}</strong><span>{String(item.selection_status).replaceAll("_", " ")} · {item.selection_responsibility === "customer" ? "Customer decision" : "Ironwood decision"} · Due {new Date(`${item.selection_deadline}T12:00:00`).toLocaleDateString()}</span></div>{item.estimates?.projects?.id && <Link href={`/projects/${item.estimates.projects.id}`}>Open</Link>}</article>)}{dueMilestones.map((milestone: any) => <article key={milestone.id}><CalendarDays/><div><strong>{milestone.title} · {money(milestone.balance)} remaining</strong><span>{milestone.due_trigger || "Payment milestone"} · Due today</span></div>{milestone.estimates?.projects?.id && <Link href={`/projects/${milestone.estimates.projects.id}`}>Open</Link>}</article>)}</div></section>}
