...
export default async function CustomerPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!c) notFound();
  return <div className="page-wrap"><PageHeader eyebrow="Customer record" title={`${c.first_name} ${c.last_name}`} description={c.company_name || "Ironwood Remodeling customer"} actions={<div className="button-row"><Link className="button button--outline" href={`/customers/${c.id}/edit`}>Edit customer</Link><Link className="button button--gold" href={`/estimates/new?customer=${c.id}`}>+ New estimate</Link></div>} />
