...
export function PublicProposal({ token, proposal, via }: { token: string; proposal: any; via?: string }) {
  ...
  return <main className="proposal-page">
    ...
    <article className="proposal-sheet">
      ...
      {proposal.payment_milestones?.length > 0 && <section className="proposal-section"><h2>Payment milestones</h2><div className="payment-milestone-summary">{proposal.payment_milestones.map((milestone: any) => { const amount = milestone.amount_type === "percentage" ? Number(estimate.total) * Number(milestone.amount_value) / 100 : Number(milestone.amount_value); return <article key={milestone.id}><div><strong>{milestone.title}</strong><b>{money(amount)}</b></div><small>{milestone.amount_type === "percentage" ? `${milestone.amount_value}% of project total` : "Fixed amount"}{milestone.due_trigger ? ` · ${milestone.due_trigger}` : ""}{milestone.due_date ? ` · Due ${new Date(`${milestone.due_date}T12:00:00`).toLocaleDateString()}` : ""}</small></article>; })}</div></section>}
