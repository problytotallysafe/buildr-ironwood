...
export function SiteVisitForm({ action, customers, projects, estimates, worksheet, defaultCustomerId = "", editable = true }: {
  ...
  return <form action={action} className="stack">
    <fieldset className="site-visit-fields stack" disabled={!editable}>
    ...
    <div className="form-actions button-row"><Link href="/site-visits" className="button button--outline">Back to site visits</Link>{editable && <><button type="submit" name="_next" value="estimate" className="button button--outline"><FilePlus2 size={16}/>Save & start estimate</button><button className="button button--gold"><Save size={16}/>{worksheet ? "Save worksheet changes" : "Save site visit"}</button></>}</div>
  </form>;
}
