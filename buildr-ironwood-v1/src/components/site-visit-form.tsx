...
export function SiteVisitForm({ action, customers, projects, estimates, worksheet }: {
  ...
  const customerId = worksheet?.customer_id ?? "";
  ...
  return <form action={action} className="stack">
    ...
    <div className="form-actions button-row"><Link href="/site-visits" className="button button--outline">Cancel</Link><button className="button button--gold"><Save size={16}/>{worksheet ? "Save worksheet changes" : "Save site visit"}</button></div>
  </form>;
}
