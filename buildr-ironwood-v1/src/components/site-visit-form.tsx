...
export function SiteVisitForm({ action, customers, projects, estimates, worksheet, defaultCustomerId = "" }: {
  ...
  return <form action={action} className="stack">
    ...
    <div className="form-actions button-row"><Link href="/site-visits" className="button button--outline">Cancel</Link><button type="submit" name="_next" value="estimate" className="button button--outline"><FilePlus2 size={16}/>Save & start estimate</button><button className="button button--gold"><Save size={16}/>{worksheet ? "Save worksheet changes" : "Save site visit"}</button></div>
  </form>;
}
